...
describe(wrapModule, () => {
  describe('BitSet callback emission', () => {
    it('does not invoke the callback for unresolved dependencies', async () => {
      ...
      const result = wrapModule(module, {
        ...options,
      ...
      expect(result.src).toContain(',[null,"/app/expo-mock/async-require"]');
    });
    ...
    it.each([`__prefetchImport('./route');`, `require.unstable_importMaybeSync('./route');`])(
      ...
      async (source) => {
        ...
        const result = await helpWrap(source, {
          chunkingStrategy: 'bitset',
          includeAsyncPaths: true,
          unstable_getAsyncDependencyPath: callback,
        });
        expect(callback).toHaveBeenCalledTimes(1);
        expect(result.paths).toEqual({ '/app/route.js': ['/route.js'] });
      }
    );
    ...
    it('uses the callback before development URLs and forwards an array unchanged', async () => {
      ...
      const result = await helpWrap(`import './sync'; import('./route');`, {
        chunkingStrategy: 'bitset',
        dev: false,
        splitChunks: true,
        includeAsyncPaths: true,
      ...
      expect(callback).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ includeAsyncPaths: true, chunkingStrategy: 'bitset' })
      );
      expect(result.paths).toEqual({ '/app/route.js': ['/sub/shared.js', '/sub/route.js'] });
      expect(result.src).toContain('"paths":{"/app/route.js":["/sub/shared.js","/sub/route.js"]}');
      expect(result.src).not.toContain('/legacy.js');
      expect(result.src).not.toContain('.bundle?');
    });
    ...
    it.each([null, undefined])(
      ...
      async (value) => {
        ...
        const result = await helpWrap(`import('./route');`, {
          chunkingStrategy: 'bitset',
          dev: false,
          splitChunks: true,
          includeAsyncPaths: true,
        ...
        expect(callback).toHaveBeenCalledTimes(1);
        expect(result.paths).toEqual({});
        expect(result.src).toContain(',["/app/route.js",');
        expect(result.src).not.toContain('"paths":');
        expect(result.src).not.toContain('/legacy.js');
        expect(result.src).not.toContain('.bundle?');
      }
    );
    ...
    it('keeps worker payloads scalar and ordinary singleton payloads as arrays', async () => {
      const result = await helpWrap(
        `require.unstable_resolveWorker('./worker'); import('./route');`,
        {
          chunkingStrategy: 'bitset',
          includeAsyncPaths: true,
      ...
      expect(result.src).toContain('"/app/worker.js":"/worker.js"');
      expect(result.src).toContain('"/app/route.js":["/route.js"]');
    });

    it('allows a weak edge to return null while retaining its module ID', async () => {
      const callback = jest.fn(() => null);
      const result = await helpWrap(`require.resolveWeak('./weak');`, {
        chunkingStrategy: 'bitset',
        includeAsyncPaths: true,
        unstable_getAsyncDependencyPath: callback,
      });
      expect(callback).toHaveBeenCalledTimes(1);
      expect(result.paths).toEqual({});
      expect(result.src).toContain('/app/weak.js');
      expect(result.src).not.toContain('"paths":');
    });

    it('keeps the legacy branch when a callback exists without the selected strategy', async () => {
      const callback = jest.fn(() => ['/new.js']);
      const result = await helpWrap(`import('./route');`, {
        includeAsyncPaths: false,
        splitChunks: true,
        unstable_getAsyncDependencyPath: callback,
      ...
      expect(callback).not.toHaveBeenCalled();
      expect(result.paths).toEqual({ '/app/route.js': '/legacy.js' });
    });
