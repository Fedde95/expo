...
export async function microBundle({
  ...
  options?: {
    dev?: boolean;
    lazy?: boolean;
    /** Manufacture weak-only graph nodes for historical legacy serializer fixtures only. */
...
`,
  ...
  async function recurseWith(queue: string[], parent?: Module, onResolve?: (fp: string) => void) {
    while (queue.length) {
      ...
      // Every edge needs its resolved path, including references to visited modules.
      onResolve?.(absPath);
