...
export function computeBitSetChunkPlan(
  ...
  const requiredByEntry = entrypoints.map(() => new Set<ChunkAtom>());
  for (const chunk of chunks) {
    for (const module of chunk.modules) {
      chunkByModule.set(module, chunk);
      for (const entryIndex of bitIndices(dependentEntriesByModule.get(module)!)) {
        requiredByEntry[entryIndex]!.add(chunk);
      }
    }
  }
  return {
