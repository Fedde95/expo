...
describe('atoms and already-loaded ownership', () => {
  ...
  it('does not mutate the graph or depend on entry/root/dependency insertion order', async () => {
    ...
    const before = [...graph.dependencies].map(([path, m]) => [
      path,
      [...m.dependencies],
      m.output,
    ]);
    const first = computeBitSetChunkPlan([entry], graph, { isLazyBundle: false });
    expect(
    ...
    const second = computeBitSetChunkPlan(
      [entry],
      { ...graph, dependencies: reversed },
      { isLazyBundle: false }
    );
    const signature = (plan: typeof first) =>
      plan.chunks.map((c) => [c.dependentEntries, [...c.modules].map((m) => m.path)]);
    expect(signature(second)).toEqual(signature(first));
    expect(owners(second, 'shared')).toEqual(['/app/a.js', '/app/b.js']);
  });
...
describe('raw entrypoint reachability', () => {
  ...
  it('tracks every entry context reaching an importer module', async () => {
    ...
    // Independent initial entries are represented as independently loadable Metro routes.
    const { entry, graph } = await loadGraph({
      'index.js': `import('./a'); import('./b');`,
