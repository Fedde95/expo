...
import * as workerScan from '../findUnsupportedWorkerAsyncDependency';
...
describe('worker compatibility', () => {
  ...
  beforeEach(() => {
    // Observe the real serializer boundary without replacing chunk emission.
  ...
  async function workerBundle(workerSource: string) {
    return microBundle({
      fs: {
  ...
  it.each(['async', 'maybeSync', 'prefetch'] as const)(
    ...
    async (asyncType) => {
      const [entry, , graph, options] = await workerBundle(`import './shared';`);
  ...
  it('finds async edges inside nested workers', async () => {
    const [entry, , graph] = await workerBundle(`require.unstable_resolveWorker('./nested');`);
    expect(workerScan.findUnsupportedWorkerAsyncDependency(entry, graph)).toEqual({
  ...
  it('ignores weak worker edges even when the fixture manufactures their target', async () => {
    const [entry, , graph] = await workerBundle(`require.resolveWeak('./shared');`);
  ...
  it('does not treat page-only async edges as worker edges', async () => {
    const [, , graph, options] = await workerBundle(`import './target';`);
    await serialize(graph, options);
  ...
  it('ignores unresolved worker edges', async () => {
    const [entry, , graph] = await workerBundle(`import('./target');`);
  ...
  it('does not scan workers when splitting is disabled', async () => {
    const [, , graph, original] = await workerBundle(`import('./target');`);
