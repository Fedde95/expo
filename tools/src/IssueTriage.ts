 ...
 * The three redirect kinds stay `null` until the shadow logs show they are right often enough.
 */
...
/** The labels maintainers use for the redirect kinds. Read as truth, never applied. */
...
const TRIAGE_QUESTION: ChoiceQuestion<Record<TriageKind, string>> = {
  ...
  criteria: {
    ...
    question:
      'The author asks how to do something, why something behaves as it does, or reports a problem in their own project with no sign of a defect in Expo code. Pick this for vague or one-line reports that describe a symptom without steps or code.',
    ...
    missing_info:
      'The template sections are blank, deleted, or hold placeholder text, so the report has no title-level description of a problem at all',
    documentation:
