...
| <kbd>J</kbd>                    | Open React Native DevTools for any connected device that is using Hermes as the JavaScript engine. See [debugging JavaScript with Hermes](/guides/using-hermes#javascript-debugger).     |
...
| `EXPO_NO_DEV_MENU`                   | **boolean** | Appends the reserved `__expo_*` query parameters that keep the developer menu closed, hide the tools button and skip onboarding to every Expo Go and development build launch URL for that session. The dev menu and the tools button come back on the next launch without the variable; only onboarding stays finished.                                                                                                        |
